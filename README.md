...
5. Using Axios to fetch Tokens and validate using XSSEC
   ...
   xsenv.loadEnv();
   oCredentials = xsenv.readServices(constants.xsuaa)[constants.xsuaa]
     .credentials;
...
- An Event-Driven Architecture for distributed systems. I call it a Shared-Queue.
- It's a "loose coupling" of applications.
- It allows businesses to grow iteratively, introducing new capabilities gradually without disrupting existing operations.
- It can also act as a Shock Absorber (Buffer) in extreme peak loads where synchronous communication is not feasible.
- 
